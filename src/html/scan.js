...
function updateUIDType(uidtype) {
  ...
  if (!uidtype || uidtype.startsWith('Not set')) // TX
  {
    bg = '#D50000';  // red/white
    uidtype = 'Not set';
    desc = 'Используется автоматически сгенерированный UID';
  }
  else if (uidtype === 'Flashed') // TX
  {
    bg = '#1976D2'; // blue/white
    desc = 'UID получен из фразы привязки, заданной при прошивке';
  }
  else if (uidtype === 'Overridden') // TX
  {
    bg = '#689F38'; // green/black
    fg = 'black';
    desc = 'UID получен из фразы, введённой в поле «фраза привязки» выше';
  }
  else if (uidtype === 'Modified') // edited here
  {
    bg = '#7c00d5'; // purple
    desc = 'UID изменён, но ещё не сохранён';
  }
  else if (uidtype === 'Volatile') // RX
  {
    bg = '#FFA000'; // amber
    desc = 'UID будет сброшен при загрузке';
  }
  else if (uidtype === 'Loaned') // RX
  {
    bg = '#FFA000'; // amber
    desc = 'Приёмник выдан во временное пользование (Lua или three-plug)';
  }
  else // RX
  {
    if (_('uid').value.endsWith('0,0,0,0'))
    {
      bg = '#FFA000'; // amber
      uidtype = 'Not bound';
      desc = 'Приёмник не привязан и загрузится в режим bind';
    }
    else
    {
      bg = '#1976D2'; // blue/white
      uidtype = 'Bound';
      desc = 'Приёмник привязан и ждёт соединение';
    }
  }
...
function updateConfig(data, options) {
  ...
  if (options && options['gerda-2g4'] !== undefined && _('reg_domain')) {
    const band = (Number(options['gerda-2g4']) === 1) ? 'CUST2640' : 'ISM2G4';
    if (data.reg_domain && data.reg_domain !== band) {
      _('reg_domain').textContent = data.reg_domain + ' / ' + band;
    } else {
      _('reg_domain').textContent = band;
    }
  }
  if (data.uid) {
